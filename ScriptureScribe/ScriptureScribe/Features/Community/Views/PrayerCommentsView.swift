//
//  PrayerCommentsView.swift
//  ScriptureScribe
//
//  Sheet showing a prayer request at the top with community replies below.
//  Supports liking, replying-to, editing, and deleting comments.
//

import SwiftUI

struct PrayerCommentsView: View {
    ...
    private var requestHeader: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 10) {
                UserAvatarView(displayName: request.displayName, photoURL: request.photoURL, size: 36)
                ...
                Spacer()
                Text("\u{1F64F} \(request.prayingCount)")
                    .font(.caption)
                    .foregroundStyle(themeManager.currentTheme.textSecondary)
            }
            Text(request.text)
    ...
    private func loadComments() async {
        isLoading = true
        comments  = (try? await firestore.fetchPrayerComments(requestId: request.id)) ?? []
    ...
    private func sendComment() async {
        ...
        do {
            try await firestore.addPrayerComment(comment, requestId: request.id)
            commentText = ""
            replyingTo  = nil
    ...
    private func toggleCommentLike(_ comment: Comment) async {
        ...
        do {
            try await firestore.toggleCommentLike(
                parentCollection: "prayerRequests", parentId: request.id,

//
//  GratitudeCommentsView.swift
//  ScriptureScribe
//
//  Sheet showing a gratitude post at the top with community comments below.
...
struct GratitudeCommentsView: View {
    ...
    private func loadComments() async {
        isLoading = true
        comments  = (try? await firestore.fetchGratitudeComments(postId: post.id)) ?? []
    ...
    private func sendComment() async {
        ...
        do {
            try await firestore.addGratitudeComment(comment, postId: post.id)
            commentText = ""
            replyingTo  = nil
    ...
    private func toggleCommentLike(_ comment: Comment) async {
        ...
        do {
            try await firestore.toggleCommentLike(
                parentCollection: "gratitudePosts", parentId: post.id,
